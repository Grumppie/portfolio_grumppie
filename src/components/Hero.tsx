...
export function Hero({
  ...
  useEffect(() => {
    ...
    const onMove = (event: MouseEvent) => {
      x.set((event.clientX - window.innerWidth / 2) / (window.innerWidth / 2))
      y.set((event.clientY - window.innerHeight / 2) / (window.innerHeight / 2))
    }
  ...
  useEffect(() => {
    ...
    const contentTimer = setTimeout(() => setRevealContent(true), 150)
  ...
  useEffect(() => {
    ...
    if (isInView) {
      ...
      const shaderTimer = setTimeout(() => setShowShader(enableShader), 30)
      const contentTimer = setTimeout(() => setRevealContent(true), 150)
      return () => {
        ...
        >
          ...
          <div style={{ transform: "translateZ(20px)" }}>
            <motion.p
              ...
              className="mt-4 text-xs md:text-sm font-mono text-white tracking-[0.3em] uppercase transition-all duration-500 group-hover:text-white"
            >
              {profile.roleLine}
            </motion.p>
          </div>
