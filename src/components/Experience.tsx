...
const experienceData = [
    {
        ...
        glowColor: [
            "radial-gradient(ellipse 120% 100% at 50% 40%, rgba(255, 20, 147, 0.35), transparent 70%)",
            "radial-gradient(ellipse 100% 120% at 30% 65%, rgba(255, 140, 0, 0.3), transparent 65%)",
            "radial-gradient(ellipse 90% 100% at 75% 55%, rgba(233, 30, 99, 0.25), transparent 60%)",
        ],
        accentColor: "#FF1493",
    ...
    },
    {
        ...
        glowColor: [
            "radial-gradient(ellipse 120% 100% at 50% 45%, rgba(108, 92, 231, 0.35), transparent 65%)",
            "radial-gradient(ellipse 100% 110% at 25% 55%, rgba(72, 52, 212, 0.3), transparent 60%)",
            "radial-gradient(ellipse 90% 100% at 75% 50%, rgba(162, 155, 254, 0.2), transparent 55%)",
        ],
        accentColor: "#6C5CE7",
    ...
    },
    {
        ...
        glowColor: [
            "radial-gradient(ellipse 120% 100% at 50% 45%, rgba(45, 27, 105, 0.4), transparent 65%)",
            "radial-gradient(ellipse 100% 110% at 35% 60%, rgba(155, 89, 182, 0.3), transparent 60%)",
            "radial-gradient(ellipse 90% 100% at 70% 50%, rgba(245, 166, 35, 0.25), transparent 55%)",
        ],
        accentColor: "#F5A623",
...
export function Experience({ onHoverGlow }: { onHoverGlow?: (color: string[] | null) => void }) {
    ...
    return (
        <section id="experience" className="relative w-full min-h-screen flex flex-col items-center justify-start overflow-hidden pb-32 -mt-[75vh] pt-[15vh]">

            {/* Background Layering */}
            <div className="absolute inset-0 z-0 pointer-events-none">
                <div className="w-full h-[75vh] bg-gradient-to-b from-transparent via-black/80 to-black" />
                <div className="w-full h-full bg-black" />
            </div>

            {/* Container */}
            <div className="relative z-10 w-full max-w-5xl px-4 md:px-12 flex flex-col items-start mt-24">
                <h2 className="text-4xl md:text-6xl font-bold tracking-tight text-white mb-16">
                    Mission Log
                </h2>

                <div className="relative border-l border-white/10 ml-4 md:ml-0 flex flex-col gap-12 pb-24">
                    {experienceData.map((item, i) => (
                        <div key={i} className="relative pl-8 md:pl-16 group" onMouseEnter={() => handleEnter(i)} onMouseLeave={handleLeave}>
                            {/* Timeline Dot */}
                            <span
                                className="absolute left-[-5px] top-2 w-[9px] h-[9px] rounded-full border transition-all duration-300 drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]"
                                style={{
                            ...
                            <a
                                ...
                                className="block bg-white/5 border border-white/10 backdrop-blur-md rounded-2xl p-8 transition-all duration-500 cursor-pointer"
                                style={{
                                    ...
                                    boxShadow: hoveredIndex === i
                                        ? `0 20px 60px -10px ${item.accentColor}30, 0 0 30px ${item.accentColor}15`
                                        : 'none',
                            ...
                            >
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                                    <div>
                                        <h3 className="text-2xl font-semibold text-white tracking-wide">{item.role}</h3>
                                        <p className="font-medium text-lg mt-1 transition-colors duration-300" style={{ color: hoveredIndex === i ? item.accentColor : 'rgb(161,161,170)' }}>
                                            {item.company}
                                            <span className="inline-block ml-2 text-xs opacity-0 group-hover:opacity-60 transition-opacity duration-300">↗</span>
                                        </p>
                                        <p className="text-zinc-300 text-sm font-mono mt-0.5">{item.location}</p>
                                    </div>
                                    <span className="text-sm font-mono text-zinc-300 bg-white/5 px-3 py-1 rounded-full border border-white/10 whitespace-nowrap">
                                        {item.date}
                                    </span>
                                </div>
